...
class WorkWithVideo:
    def __init__(self, **kwargs):
        if not kwargs.get('source_path'):
            raise Exception('No source path')
        self.source_path = kwargs.get('source_path')

        def _open_video_container(self, source_path, mode, options=None):
            return av.open(source_path, mode=mode, options=options)
...
class UploadedMeta(PrepareInfo):
    ...
    def frame_sizes(self):
        ...
        video_stream = self._get_video_stream(container)
        container.seek(offset = next(iter(self.key_frames.value())), stream = video_stream)
        for packet in container.demux(video_stream):
            for frame in packet.decode():
                self._close_video_container(container)
                return (frame.width, frame.height)
...
def prepare_meta(media_file, upload_dir=None, meta_dir=None, chunk_size=None):
    ...
    meta_info = PrepareInfo(source_path = paths.get('source_path'), meta_path = paths.get('meta_path'))
