# -*- coding: utf-8 -*-
"""
@Time    : 11/5/20 12:36 PM
@Author  : Lucky
@Email   : lucky_soft@163.com
@File    : media_extractors.py
...
def create_tmp_dir():
    return tempfile.mkdtemp(prefix='labelhub-', suffix='.data')
...
class PdfReader(ImageListReader):
    def __init__(self, source_path, step = 1, start = 0, stop = None):
        ...
        paths = convert_from_path(self._pdf_source, last_page=stop, paths_only=True, output_folder=self._tmp_dir, fmt='jpeg', output_file=_make_name())
...
class IChunkWriter(ABC):
    ...
    def _compress_image(image_path, quality):
        ...
        # ensure image data fits into 8bit per pixel before RGB conversation as PIL clips values on conversation
        if image.mode == 'I':
...
def _is_image(path):
    ...
    return mime[0] is not None and mime[0].startswith('image') and not mime[0].startswith('image/svg')
